...
class Tournament(object):
    ...
    def play_tournament(self):
        ...
        print "Minimum, average and maximum grand total:", self.get_minimum("gt"), self.get_average(
            "gt"), self.get_maximum("gt")
...
def import_engine(arg):
    ...
    try:
        engine_module = __import__("engines.%s" % engine_name,
                                   fromlist=["engines"])  # fromlist defines the package from which to import
        e = engine_module.Engine(engine_name,
                                 *engine_args)  # Instantiate an Engine from the desired module, with the desired arguments. The length of engine_args should match the number of positional arguments in the engine constructor.
    except ImportError:
...
def CLParser():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Let's play some Yeahtzee!",
                                     epilog="And that, my friends, is how it's done.")
    ...
    parser.add_argument("-e", "--engines", nargs="*", default=[],
                        help="Engines participating in the tournament. Options for an engine can be appended with dots.")
    arguments = parser.parse_args()

    try:
        assert arguments.engines
