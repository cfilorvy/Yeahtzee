...
class Engine(object):
    ...
    def start_to_run_complete(self):
        ...
        while self.grid.available_positions():
            ...
            if len(diff) == 0:
                logging.error(
                    "Engine {0.name} is cheating: no position was filled.\nPreviously filled positions:\t{1}\nCurrently filled positions:\t{2}".format(
                        self, previously_filled_positions, currently_filled_positions))
                sys.exit()
            elif len(diff) > 1:
                logging.error(
                    "Engine {0.name} is cheating: several position were changed.\nPreviously filled positions:\t{1}\nCurrently filled positions:\t{2}".format(
                        self, previously_filled_positions, currently_filled_positions))
                sys.exit()
